...
SELECT DEPARTMENT , SALARY, RANK() OVER(ORDER BY SALARY DESC) RANK,
ROW_NUMBER() OVER(PARTITION BY  DEPARTMENT  ORDER BY SALARY DESC) ROWNUMBER ,
PERCENT_RANK() OVER(PARTITION BY  DEPARTMENT  ORDER BY SALARY DESC) PERCTRANK
FROM STAFF ORDER BY 3 ASC FETCH FIRST 1 ROWS ONLY;


SELECT MAX(SALARY) FROM STAFF;

SELECT DISTINCT NTH_VALUE(SALARY,1) OVER(ORDER BY SALARY DESC ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING ) AS "TOP SALARY" FROM STAFF;
SELECT DISTINCT NTH_VALUE(SALARY,1) OVER(ORDER BY SALARY DESC RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING ) AS "TOP SALARY" FROM STAFF;
